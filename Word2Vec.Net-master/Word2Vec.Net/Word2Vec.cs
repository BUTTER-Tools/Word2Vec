    ...
    public class Word2Vec
    {
            ...
            )
        {
            ...
            for (var i = 0; i < ExpTableSize; i++)
            {
                ...
                //fix adopted from https://github.com/eabdullin/Word2Vec.Net/pull/13/commits/fd628ddb07685ca095aa0c965a58e263479de864
            ...
            using (var fin = File.OpenText(_trainFile))
                ...
                while ((line = fin.ReadLine()) != null)
                    ...
                    foreach (var word in words)
                    {
                        ...
                        {
                            StatusToReport = string.Format("Training Words: {0}K", _trainWords/1000);
                    ...
                    if (_debugMode > 0)
                    {
                        StatusToReport = string.Format("Vocab size: {0} | ", _vocabSize);
                        StatusToReport += string.Format("Words in train file: {0}", _trainWords);
                    }
                    //file_size = ftell(fin);
            ...
            using (StreamReader fi = File.OpenText(_trainFile))
                {
                    ...
                    if (wordCount - lastWordCount > 10000)
                        ...
                        if (_debugMode > 1)
                            {
                            StatusToReport = string.Format("Alpha: {0}  Progress: {1:0.00}%  Words/thread/sec:{2:0.00}K ", _alpha,
        ...
        {
            long d;
            StatusToReport = string.Format("Starting training using file {0} | ", _trainFile);
            ...
            using (var stream = new FileStream(_outputFile, FileMode.Create, FileAccess.Write))
            {
                    ...
                    for (var a = 0; a < _vocabSize; a++)
                            {
                        var bytes = string.Format("{0} {1}", _vocab[a].Word, cl[a]).GetBytes();
                                stream.Write(bytes, 0, bytes.Length);
                            }
